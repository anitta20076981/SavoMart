...
var SavoMartApp = function () {
    ...
    var createButtons = function () {
        ...
        buttonsGroup.map(function (group) {
            ...
            // Toggle Handler
            SavoMartUtil.on(group, selector, 'click', function (e) {
    ...
    const createTagify = () => {
        ...
        new Tagify(tagify, SavoMartJson.options.tagify);
    }
    ...
    var createCountUp = function () {
        ...
        elements.map(function (element) {
            if (SavoMartUtil.isInViewport(element) && SavoMartUtil.visible(element)) {
                if (element.getAttribute("data-kt-initialized") === "1") {
                    return;
                }
    ...
    var createTinySliders = function () {
        ...
        var initSlider = function (el) {
            ...
            if (el.closest('.tns')) {
                SavoMartUtil.addClass(el.closest('.tns'), 'tns-initiazlied');
            }
            ...
            new SmoothScroll('a[data-kt-scroll-toggle][href*="#"]', {
                ...
                offset: function (anchor, toggle) {
                    ...
                    if (anchor.hasAttribute('data-kt-scroll-offset')) {
                        var val = SavoMartUtil.getResponsiveValue(anchor.getAttribute('data-kt-scroll-offset'));
    ...
    var initCheck = function () {
        ...
        // Toggle Handler
        SavoMartUtil.on(document.body, '[data-kt-check="true"]', 'change', function (e) {
            var check = this;
            var targets = document.querySelectorAll(check.getAttribute('data-kt-check-target'));

            SavoMartUtil.each(targets, function (target) {
    ...
    var initBootstrapCollapse = function () {
        ...
        SavoMartUtil.on(document.body, '.collapsible[data-bs-toggle="collapse"]', 'click', function (e) {
    ...
    var initBootstrapRotate = function () {
        ...
        SavoMartUtil.on(document.body, '[data-kt-rotate="true"]', 'click', function (e) {
    ...
    const initNoUIslider = () => {
        const sliderElements = [].slice.call(document.querySelectorAll('[data-kt-noUiSlider="true"]'));
        if (sliderElements && $(sliderElements).length) {
            sliderElements.map(function (element) {
                ...
                noUiSlider.create(element, {
                    start: [$(elementParent).find('[data-kt-noUiSlider-value="true"]').val()],
                    connect: true,
                    step: 1,
                    range: {
                        "min": 0,
                ...
                element.noUiSlider.on("update", function (values, handle) {
                    $(elementParent).find('[data-kt-noUiSlider-span="true"]').html(Math.round(values[handle]));
                    $(elementParent).find('[data-kt-noUiSlider-value="true"]').val(Math.round(values[handle]));
                });
            });
        }
    }
...
SavoMartApp.createSelect2 = function () {
    // Check if jQuery included
    if (typeof jQuery == 'undefined') {
        return;
    }
    ...
    elements.map(function (element) {
        ...
        var options = SavoMartJson.options.select2.normal;
        ...
        if (element.getAttribute('data-server') == 'true') {
            options = SavoMartJson.options.select2.server;
    ...
    if (SavoMartApp.select2FocusFixInitialized === false) {
        SavoMartApp.select2FocusFixInitialized = true;
...
SavoMartApp.handleFileRemove = function (e) {

    const removeButton = [].slice.call(document.querySelectorAll('[data-kt-file-input-action="remove"]'));
    if (removeButton && $(removeButton).length) {
        removeButton.map(function (element) {
            element.addEventListener('click', function () {
                ...
                $(fileInputParent).find('[name="' + $(fileInputParent).find('[type="file"]').attr("name") + '_remove"]').val(1);
            });
        });
    }
}
