...
var SavoMartDrawer = function (element, options) {
    ...
    var _toggle = function () {
        if (SavoMartEventHandler.trigger(the.element, 'kt.drawer.toggle', the) === false) {
            return;
        }
    ...
    var _hide = function () {
        if (SavoMartEventHandler.trigger(the.element, 'kt.drawer.hide', the) === false) {
            return;
        }
        ...
        SavoMartEventHandler.trigger(the.element, 'kt.drawer.after.hidden', the) === false
    }

    var _show = function () {
        if (SavoMartEventHandler.trigger(the.element, 'kt.drawer.show', the) === false) {
            return;
        }
    ...
    var _update = function () {
        ...
        // Reset state
        if (SavoMartUtil.hasClass(the.element, the.options.baseClass + '-on') === true && String(document.body.getAttribute('data-kt-drawer-' + the.name + '-')) === 'on') {
            the.shown = true;
        } else {
            the.shown = false;
        }
        ...
        if (_getOption('activate') === true) {
            ...
            SavoMartUtil.addClass(the.element, the.options.baseClass + '-' + direction);
            ...
            SavoMartUtil.removeClass(the.element, the.options.baseClass + '-' + direction);
    ...
    var _createOverlay = function () {
        if (_getOption('overlay') === true) {
            ...
            SavoMartUtil.css(the.overlayElement, 'z-index', SavoMartUtil.css(the.element, 'z-index') - 1); // update
...
SavoMartDrawer.hideAll = function (skip = null, selector = '[data-kt-drawer="true"]') {
...
SavoMartDrawer.handleShow = function () {
    ...
    SavoMartUtil.on(document.body, '[data-kt-drawer-show="true"][data-kt-drawer-target]', 'click', function (e) {
        e.preventDefault();
...
SavoMartDrawer.handleDismiss = function () {
    // External drawer toggle handler
    SavoMartUtil.on(document.body, '[data-kt-drawer-dismiss="true"]', 'click', function (e) {
